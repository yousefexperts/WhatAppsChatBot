...
namespace WebWhatsappAPI
{
    public abstract class IWebWhatsappDriver
    {
        ...
        public void SendMessage(string message, string person = null)
        {
			    ...
			    Console.WriteLine("Value = :: " + outp);
			    var chatbox = driver.FindElement(By.TagName("footer")).FindElements(By.TagName("div"));

			foreach (var sel in chatbox)
			{
				/*Console.WriteLine(sel.Text);*/
				var attr = sel.GetAttribute("class");
				Console.WriteLine("Value = :: " + attr);
				if (attr.Equals("_3u328 copyable-text selectable-text")) {
					Console.WriteLine("Value = :: " + attr);

					Console.WriteLine("Value = :: " + sel.Text);
					Actions actions = new Actions(driver);
					/*selectDiv.Click();
					selectDiv.SendKeys(outp);
					selectDiv.SendKeys(Keys.Enter);*/

					actions.MoveToElement(sel);
					actions.Click();
					actions.SendKeys(outp);
		...
		public  string SendMessageByNumber(string number, string message)
		{
			var variables = $@"
...
";
		...
		public void SetActivePerson(string person)
        {
			...
			IReadOnlyCollection<IWebElement> AllChats = driver.FindElement(By.Id("pane-side")).FindElements(By.ClassName("_19RFN"));
        ...
        public IEnumerable<string> GetAllChatNames()
        {
            ...
            {
				var Title = we.Text;
               yield return we.GetAttribute("title");
            }
        }
